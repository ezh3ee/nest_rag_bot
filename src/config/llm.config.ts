...
import type { EmbeddingProviderConfig, EnvConfig, GenerationProviderConfig } from './llm.schema';
...
function toGenerationProviderConfig(env: EnvConfig): GenerationProviderConfig {
  ...
  if (!result.success) {
    throw new Error(
      `[LLM Config]: Invalid generation provider config - ${formatZodIssues(result.error)}`,
    );
  }
  return result.data;
}
...
function toEmbeddingProviderConfig(env: EnvConfig): EmbeddingProviderConfig {
  ...
  if (!result.success) {
    throw new Error(
      `[LLM Config]: Invalid embedding provider config - ${formatZodIssues(result.error)}`,
    );
  }
  return result.data;
}
...
export default registerAs('llm', (): AppConfig => {
  let env: EnvConfig;

  try {
    env = llmSchema.parse(process.env);
  } catch (error) {
    if (error instanceof ZodError) {
      throw new Error(`[LLM Config]: Validation failed - ${formatZodIssues(error)}`, {
        cause: error,
      });
    }
    throw error;
